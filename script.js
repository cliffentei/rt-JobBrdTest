document.addEventListener("DOMContentLoaded", function () {
  ...
  function loadConfig() {
    return fetch("./config.json")
      ...
      .then((config) => {
        personalAccessToken = config.API_KEY;
      })
      .catch((error) => {
  ...
  loadConfig().then(() => {
    ...
    async function fetchFilterOptions() {
      ...
      const fetchedExperiences = await fetchTableRecordsWithOffset(
        "Experience"
      );
    ...
    const resizeObserver = new ResizeObserver((entries) => {
      for (const entry of entries) {
        ...
        if (newHeight + 20 !== originalHeight) {
          paginElement.style.marginTop = `-${
            newHeight + 20 - originalHeight
          }px`;
        } else {
    ...
    function urlCreator(s) {
      const baseUrl =
        "https://api.airtable.com/v0/apprdsx9uO4l5FieL/Table%201?";
      ...
      filterFunction += `${encodeURIComponent(
        "AND(" + filters.join(",") + ")"
      )}`;
        ...
        fetch(newUrl, {
          ...
          .then((data) => {
            ...
            setTimeout(() => {
              ...
              htmlString =
                '<div id="toDelete" class="card-holder">' +
                htmlString +
                "</div>";
